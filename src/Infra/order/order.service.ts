...
export class OrderService {
	constructor(
		@Inject(AddItemToCustomBasketUseCase)
		private readonly addItemToCustomBasketUseCase: AddItemToCustomBasketUseCase,
		@Inject(RemoveItemFromCustomBasketUseCase)
		private readonly removeItemFromCustomBasketUseCase: RemoveItemFromCustomBasketUseCase
	) {}
	...
	async removeItemFromCustomBasket(
		dto: RemoveItemFromCustomBasketDto
	): Promise<void> {
		const result = await this.removeItemFromCustomBasketUseCase.execute(
			dto
		);
		this.checkResult(result);
	}
