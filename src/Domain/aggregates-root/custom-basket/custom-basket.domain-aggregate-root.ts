...
import { CustomBasketProps } from './custom-basket.domain-aggregate-root-interface';
import {
	Currency,
	ImageValueObject,
...
export class CustomBasket extends AggregateRoot<CustomBasketProps> {
	...
	private calculateTotalExchangesFactorOnRemovedItems(): number {
		...
		const total = itemsRemoved.reduce(
			(total, item) =>
				item.value.quantity.value * item.value.exchangeFactor.value +
				total,
			0
		);
