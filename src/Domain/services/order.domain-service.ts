...
import { AddProps } from './interfaces/order.domain-service.interface';
import { RemoveProps } from './interfaces/order.domain-service.interface';
import { OrderDomainServiceInterface } from './interfaces/order.domain-service.interface';

@Injectable()
export class OrderDomainService
	implements OrderDomainServiceInterface {
	constructor (
	...
	addItemToCustomBasket (props: AddProps): Result<void> {
		...
		const addedItemOrError = this.customBasketDomainService.addItemToCustomBasket(
			{
				customBasket,
				item,
				quantityToAdd
	...
	removeItemFromCustomBasket (props: RemoveProps): Result<void> {
		...
		const removedItemOrError = this.customBasketDomainService.removeItemFromCustomBasket(
			{
				customBasket,
				item,
				quantityToRemove
